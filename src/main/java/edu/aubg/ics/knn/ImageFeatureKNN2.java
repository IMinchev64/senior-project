...
public class ImageFeatureKNN2 {
    public static List<LabelPercentagePair> findNearestLabel(float[] features, int k, Connection connection) throws SQLException {
        final int n = 12000;
        ...
        labelsWithPercentages.sort(Comparator.comparingDouble(LabelPercentagePair::getPercentage).reversed());

        System.out.println("Labels with percentages:");
        for (LabelPercentagePair pair : labelsWithPercentages) {
            System.out.printf("Label: %s, Percentage: %.2f%%, Weight: %.6f%n", pair.getLabel(), pair.getPercentage(), pair.getDistance());
        }

        return labelsWithPercentages;
    }
